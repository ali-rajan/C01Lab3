...
function Dialog({
  ...
  // For some reason entry is an event rather than a note entry
  const patchNote = async (entry) => {
    try {
      // TODO: find if there's a better way than URL string concatenation
      await fetch('http://localhost:4000/patchNote/' + note._id, {
        ...
        if (!response.ok) {
          ...
          // TODO: find a better way to do this rather than listing all attributes
